...
router.get("/", async (req, res) => {
    ...
    const total_categories = await ProductCategory.count({ where }).catch(console.trace);
    req.App
        .assignData("categories", categories)
        .assignData("total_results", total_categories)
        .assignData("pager_current_page", Number(page))
        .assignData("pager_total_pages", Math.ceil(total_categories / limit))
